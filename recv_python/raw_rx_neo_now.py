...
if data_conf['output_fft']:
    ...
    fft_id_to_file = np.zeros((n_blocks_to_save, n_frames_per_loop),
                              dtype=np.uint32)


def dumpdata_hdf5_raw(data_dir, data_conf, data, id_data, block_time):

    global voltage_fail, abs_fail_volt
    global file_cnt, tot_file_cnt
    ...
    dset = f.create_dataset('block_time', data=block_time_str)
...
def dumpdata_hdf5_fft_spectrum(data_dir, data, id_data):
    ...
    if i2 == n_blocks_to_save:
        ...
        file_path = data_file_prefix(data_dir, block_time, data_conf)
        fout = os.path.join(file_path, labels[output_sel] + '_' + str(k))
        ...
        if file_path == file_path_old:
            ...
            logging.info("Congrate! Next hour, new sub-directory: " +
                         file_path)
...
if __name__ == '__main__':
    ...
    logging.info("Warmup finished with last data seqNo " +
                 "and the remainder of SeqNo/block_size: %i, %i ",
                 id_tail_before, tmp_id % block_size)
    if log_ngrp:
        print("Warning: n_blocks_to_save is changed to ", ngrp)
        logging.warning("Warning: n_blocks_to_save is changed to %i ", ngrp)

    try:
        sock_temp = socket.socket(family=socket.AF_INET,
    ...
    try:
            ...
            while forever:
                ...
                if (diff == 1) or (diff == -cycle):
                    ...
                    if (num_lost_p > 0):
                        ...
                        if save_lost:
                            if data_conf['output_fft']:
                                wfile = executor.submit(
                                    dumpdata_hdf5_fft_spectrum,
                    ...
                    logging.warning("block is not connected, %i, %i",
                                    id_tail_before, id_arr[0])
                    print("block is not connected, %i, %i",
                          id_tail_before, id_arr[0])
                    num_lost_all += 1
        ...
        #######################################################################
                ...
                if time_now - time_last > 10.0:
                    ...
                    time_last = time.perf_counter()
                    live.update(
                        display_metrics_rich(time_before, time_now,
                                             s_time, num_lost_all,
                                             data_conf,
