...
if data_conf['output_fft']:
    sys.exit("wrong program, please use raw_rx_fft.py or change output_fft to True")
...
def save_raw_data(rx, dconf, v):  #{{{
    ...
    raw_data_to_file = np.zeros((n_blocks_to_save,
        data_size*n_frames_per_loop//2))
    ...
    while loop_forever:
        ...
        if (diff == 1) or (diff == - cycle ):
            ...
            if (num_lost_p > 0):
                ...
                raw_data = np.frombuffer(udp_data, dtype=data_type)

                if nn == 0:
                    # print("bt: ", raw_block_time_to_file[nn])
                ...
                raw_data_to_file[nn,...] = raw_data
                nn +=1

                if nn == n_blocks_to_save - 5: 
                    if wstart:
                        wfile.result()
                        wstart = False
